...
public class MFConfig {
    ...
    static {
        ...
        final Path path = FabricLoader.getInstance().getConfigDir().resolve( TerrainsDefaults.NAMESPACE + ".properties");
        ...
        try (OutputStream out = Files.newOutputStream(path, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING)) {
            newProperties.store(out, TerrainsDefaults.NAMESPACE.replace('_', ' ') + " config");
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    private static void logError(String key) {
        MapleForest.LOGGER.error("Failed to parse variable '" + key + "' in " + TerrainsDefaults.NAMESPACE + "'s config, generating a new one!");
    }
